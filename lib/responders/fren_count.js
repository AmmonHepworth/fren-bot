const { WAIT_DELAY, ALERT_COOLDOWN, SCREAM_ALLOWANCE } = process.env;
...
class FrenCountResponder {
  ...
  onJoin(memberName, memberCount) {
    console.log(`${memberName} joined, there are now ${memberCount} members`);

    if (this.timeSinceLastJoined(memberName) < WAIT_DELAY) {
      console.log(
        `member ${memberName} has left too recently to be counted again`
      );
  ...
  onLeave(memberName, memberCount) {
    const justScreamed =
      this.timeSinceLastJoined(memberName) < SCREAM_ALLOWANCE &&
      memberCount === 0;
