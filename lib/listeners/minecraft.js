...
function formatPlayer({ uuid, name_clean: name }) {
  return `${name} (${uuid})`;
}
...
class MinecraftListener extends BaseListener {
  ...
  async rehydrate() {
    try {
      ...
      const {
        players: { list: fullPlayers },
      } = await mcs.statusJava(this.host, this.port, { query: true });

      const nextPlayers = fullPlayers.map(formatPlayer);
